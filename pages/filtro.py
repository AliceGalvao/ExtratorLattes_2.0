...
layout = html.Div([
    ...
    html.Div([
        html.Div([
            ...
            html.Span("Lattes", style={'color': 'white', 'fontWeight': 'bold'})
                  ...
                  'padding': '20px', 'fontSize': '190%', 'fontFamily': 'Arial'}),
    ...
    html.Div([
        ...
        html.Div([
            ...
            ], style={'backgroundColor': '#D9D9D9', 'padding': '15px', 'border': '1px solid black',
                      'borderRadius': '5px'}),
        ], style={'width': '30%'}),
        ...
        html.Div([
            ...
            html.Div([
                html.H4("Outros"),
                dcc.Checklist(
                    options=[{'label': 'Eventos Organizados', 'value': 'eventos_organizados'}],
                    id='filtro-outros',
                    inline=False,
                    inputStyle={"margin-right": "10px"},
                )
              ...
              'backgroundColor': '#6EA1C2', 'padding': '40px',
              'minHeight': 'calc(100vh - 180px)'}),

    html.Div([
        html.Div("Ano de corte:", style={'color': 'white', 'marginRight': '20px'}),
        dcc.Dropdown(
            options=[{'label': str(year), 'value': year} for year in range(datetime.now().year - 10, datetime.now().year + 1)],
    ...
    ], style={'backgroundColor': '#001F3F', 'padding': '20px', 'display': 'flex',
    ...
    dcc.Store(id='store-lista-dfs'),
    dcc.Location(id='redirect-div-after-process', refresh=True)
])

@callback(
    Output('loading-div', 'children'),  # dispara o loading fullscreen
...
def processar_resultado(n_clicks, uploaded_file_data_dict, ano_corte,
                        o1, o2, o3, registro, publicacoes, outros):

    loading_trigger = "processando..."

    if not uploaded_file_data_dict:
        return "", no_update, True, "Por favor, carregue um arquivo na tela inicial."

    if not ano_corte:
        return "", no_update, True, "Por favor, selecione um ano de corte."
    ...
    try:
        ...
        if outros and 'eventos_organizados' in outros:
        ...
        if not metricas_para_leitor:
            return "", no_update, True, "Selecione pelo menos uma métrica para continuar."

        leitor = Leitor()
        lista_dfs = leitor.gerar_estrutura_de_csv_programas(ano=ano_corte, metricas=metricas_para_leitor)

        return "", lista_dfs, False, ""

    except Exception as e:
        logger.error(f"[X] Erro ao gerar resultados: {str(e)}\n{traceback.format_exc()}")
        return "", no_update, True, f"Erro durante a extração: {e}"
...
def alternar_marcar_tudo(n_clicks, orientacao_1_valores):
    if orientacao_1_valores:
        ...
        return (['coorientacao', 'orientacao'],
                ['concluido', 'andamento'],
